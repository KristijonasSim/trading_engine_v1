...
class AiHypothesisee91dcc8a4(IStrategy):
    timeframe = "5m"
    can_short = False
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        dataframe["mid"] = dataframe["close"].rolling(40).mean()
        dataframe["std"] = dataframe["close"].rolling(40).std()
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        dataframe.loc[(dataframe["close"] >= dataframe["mid"]) | (dataframe["rsi"] > 57), "exit_long"] = 1
        return dataframe
