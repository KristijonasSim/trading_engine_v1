...
class AiHypothesisfc7acd6164(IStrategy):
    timeframe = "1h"
    can_short = False
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        dataframe["mid"] = dataframe["close"].rolling(57).mean()
        dataframe["std"] = dataframe["close"].rolling(57).std()
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        dataframe.loc[(dataframe["close"] >= dataframe["mid"]) | (dataframe["rsi"] > 48), "exit_long"] = 1
        return dataframe
