async function loadEngineNav() {
  ...
  nav.innerHTML = `
    ...
    <a class="engine active engine-link" href="/testing.html"><p class="label">03 · TESTING ENGINE</p><h2>${data.testing.strategies.filter(item => ["passed_goal","failed_goal"].includes(item.status)).length} tested strategies</h2><p>Freqtrade timeframe comparison.</p><span class="badge">Ready</span></a>
