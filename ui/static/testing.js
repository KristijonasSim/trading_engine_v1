...
async function loadTesting() {
  ...
  const candidates = data.testing.strategies; const tested = candidates.filter(item => item.status === "tested").length;
  ...
  for (const strategy of candidates) {
    ...
    cell(row, strategy.name); cell(row, strategy.kind); cell(row, strategy.timeframe); cell(row, strategy.status === "tested" ? `Best of ${strategy.comparison_timeframes.join(", ")}` : `${strategy.completed_timeframes}/${strategy.comparison_timeframes.length} tested`); cell(row, metrics.score); cell(row, metrics.pf); cell(row, metrics.cagr); cell(row, metrics.max_drawdown); cell(row, metrics.trades_per_day);
